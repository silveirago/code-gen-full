...
#define N_MUX mux-num  // number of multiplexers

// Define s0, s1, s2, s3, and x pins
const int mux_s[4] = { mux-sPin };
const int mux_x[N_MUX] = { mux-xPin };  // analog pin of the first mux
...
const byte N_BUTTONS = button-total;                          //  total numbers of buttons. Number of buttons in the Arduino + number of buttons on multiplexer 1 + number of buttons on multiplexer 2... (DON'T put Octave and MIDI channel (bank) buttons here)

const byte N_BUTTONS_ARDUINO = button-num;                             // number of buttons connected straight to the Arduino
const byte BUTTON_ARDUINO_PIN[N_BUTTONS_ARDUINO] = { button-pin };  // pins of each button connected straight to the Arduino

// - - - - - - - - - - - - - - - - - - - - - - - - - - - -

#ifdef USING_MUX                                      // Fill if you are using mux, otherwise just leave it
const byte N_BUTTONS_PER_MUX[N_MUX] = { num-muxButtons };  // number of buttons in each mux (in order)
const byte BUTTON_MUX_PIN[N_MUX][16] = { mux-buttonPin };
...
const byte N_POTS = pot-total;  // total numbers of pots (slide & rotary). Number of pots in the Arduino + number of pots on multiplexer 1 + number of pots on multiplexer 2...

const byte N_POTS_ARDUINO = pot-num;  // number of pots connected straight to the Arduino
// If using the Arduino declare as "A1, A2"
// If using ESP32 only use the GPIO number as "11, 10"
const byte POT_ARDUINO_PIN[N_POTS_ARDUINO] = { pot-pin };  // pins of each pot connected straight to the Arduino (don't use "A" if you are using ESP32, only the number)

// - - - - - - - - - - - - - - - - - - - - - - - - - - - -

#ifdef USING_MUX
const byte N_POTS_PER_MUX[N_MUX] = { num-muxPots };  // number of pots in each multiplexer (in order)
const byte POT_MUX_PIN[N_MUX][16] = { mux-potPin };
#endif
...
byte MESSAGE_TYPE_POT[N_POTS] = { pot-msg-type };

byte POT_CC_N[N_POTS] = { pot-msg-val };  // Add the CC, or PB, NUMBER or MACKIE of each pot you want
...
const float encSensitivity = encoder-sensitivity;  // change the encoder sensitivity here
...
const byte N_ENCODERS = encoder-num;     // number of encoders
const byte N_ENCODER_PINS = N_ENCODERS;  //number of pins used by the encoders

//Encoder encoder[N_ENCODERS] = { { 10, 16 } };  // the two pins of each encoder -  Use pins with Interrupts!
Encoder encoder[N_ENCODERS] = { encoder-pin };  // the two pins of each encoder -  Use pins with Interrupts!

// Choose the CC or MACKIE value
byte ENCODER_CC_N[N_ENCODERS] = { encoder-msg-val };  // Add the CC NUMBER of each encoder you want
...
/////////////////////////////////////////////
// MIDI CHANNEL
byte POT_MIDI_CH = pot-midi-ch;  // MIDI channel to be used
byte BUTTON_MIDI_CH = button-midi-ch;
byte ENCODER_MIDI_CH = encoder-midi-ch;
